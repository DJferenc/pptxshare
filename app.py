...
def upload():
    ...
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                # Feltöltés Supabase-be
                with open(temp_path, "rb") as f:
                    supabase.storage.from_("default").upload(f"{session['username']}/{filename}", f, {"upsert": True})
                flash(f"Sikeres feltöltés a Supabase Storage-ba: {filename}")
